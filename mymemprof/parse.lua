...
local function parse_event(reader, events, symbols)
    qty_event = qty_event + 1
    if (qty_event % 736155) == 0 then
        local pos = tonumber(reader:bytes_readed())
        print('Current pos ' .. pos)
    end

    local ev_header = reader:read_octet()

    if not ev_header_is_valid(ev_header) then
        print("ERROR: Bad ev_header " .. ev_header)
        return false
    end
...
function M.parse(reader, symbols)
    ...
    reader.bytes_readed = function(reader)
        local ffi = require "ffi"
        ffi.cdef[[
            long ftell(FILE *stream);
        ]]
        local ffi_C = ffi.C
        -- позиция в файле = уже выдано + ещё в буфере с начала чтения
        -- через ftell можно получить реальное положение в файле:
